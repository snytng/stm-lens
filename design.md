...
*   **`StmAnalysisView`**: プラグインのエントリーポイントとなるメインパネル。`BorderLayout`を採用。
    ...
    *   **Test Panel (折りたたみ領域)**: トップパネル直下に配置。デフォルトは非表示。テストの記録・再生関連のボタン（Record, Stop, Play等）とコンボボックスを配置。
...
*   **`SimulationEngine`**: シミュレーションの中核ロジック。
    ...
    *   `getAvailableTransitions()`: 現在の状態から遷移可能な `ITransition` のリストを返す。親状態も含め、通常の遷移（`getOutgoings()`）に加え、内部遷移（`getInternalTransitions()`）も収集する。
