...
### 3.5. テスト記録・再生の処理
*   **リアルタイム記録 (Record)**:
    *   シミュレーション開始 (`Start`) と同時に自動的に開始される。
    *   `GenerationMode/Live` 状態では、イベント遷移実行のたびに `Fire:` と `AssertState:` がエディタに自動追記される。
    *   タイムトラベルで過去 (`Review`) に戻り、そこから新しいイベントを発火させた場合、それ以降の未来の履歴は破棄され、新しい分岐としてスクリプトが更新される。
*   **保存と再生**:
...
#### 設計の合理性：Live（同期）と Viewing（閲覧）
「Zero Manual Edit」の哲学に基づき、エディタが自動更新される状態と、保護される状態の2つに集約する。

1.  **Live (同期・生成状態)**:
    *   目的：操作をスクリプトに即時反映させる。
    *   振る舞い：イベント発火のたびに `Fire:` と `AssertState:` が追記される。
2.  **Viewing (閲覧・保護状態)**:
    *   目的：過去の履歴やロードした資産を安全に閲覧する。
    *   振る舞い：エディタは Read-Only となり、背景色で保護状態を示す。
    *   **合流ロジック**: この状態でイベントを発火させると、現在地までの履歴を正解としてスクリプトを再構築し、`Live` 状態へ遷移して記録を再開する。

#### UI状態マシンの詳細
UIの具体的な状態遷移図は、以下のスクリプトを astah* で実行することで確認できる。
*   `c:\tools\astah-plugins\stm\test\scripts\design_UI.js`

このスクリプトが生成する図を本プラグインの正解の設計（Master Design）とする。
