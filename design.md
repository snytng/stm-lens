...
*   **`StmAnalysisView`**: プラグインのエントリーポイントとなるメインパネル。`BorderLayout`を採用。
    ...
    *   **Test Panel (折りたたみ領域)**: トップパネル直下に配置。デフォルトは非表示。
        *   **上部**: テストの記録・再生関連のボタン（Record, Stop, Play等）とコンボボックスを配置。
        *   **下部 (JSplitPane)**: テキストベースの自動テスト用エリア。左側にスクリプト入力欄（`JTextArea`）、右側に結果表示欄（`JTextArea`）を左右分割で配置し、高さを抑える。
