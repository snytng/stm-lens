...
#### 準備
*   任意のステートマシン図（例：`TestModel_No25`）を開いておく。

#### テストケース

##### Case 43-1: シミュレーション開始 (Idle -> Live)
*   **操作**: `Start` ボタンをクリック。
*   **確認事項**:
    1.  エディタの背景色が **白 (White)** であること。
    2.  エディタが **ReadOnly**（手入力不可）であること。
    3.  イベントボタンを押すと、スクリプトが末尾に自動追記されること。

##### Case 43-2: スクリプトのロード (Idle -> Viewing)
*   **操作**: `Reset` 後、保存済みのテストを選択して `[Load]` をクリック。
*   **確認事項**:
    1.  エディタの背景色が **薄いグレー (Light Gray)** に変わること。
    2.  エディタが **ReadOnly** であること。

##### Case 43-3: タイムトラベルによる保護 (Live -> Viewing)
*   **操作**: `Start` して数回遷移させた後、ナビゲーションの `[<]` (戻る) をクリック。
*   **確認事項**:
    1.  最新ステップより前に戻った瞬間、背景色が **薄いグレー** に変わること。
    2.  ステップ表示ラベルが更新されること。

##### Case 43-4: 閲覧状態からの合流（Branching） (Viewing -> Live)
*   **操作**: `Case 43-2` (Load後) または `Case 43-3` (過去へ戻った後) の状態で、**イベントボタン**をクリック。
*   **確認事項**:
    1.  背景色が即座に **白** に戻ること。
    2.  エディタの内容が「現在地までの履歴」で確定（再構築）され、新しいイベントが追記されること。
    3.  これ以降、再び自動同期（Live）状態になること。

##### Case 43-5: テスト実行完了後の遷移 (TestRunningMode -> Viewing)
*   **操作**: スクリプトを `[▶ Run Script]` で実行し、完了を待つ。
*   **確認事項**:
    1.  実行中、エディタは **薄いグレー**（または実行中を示す色）で保護されていること。
    2.  完了後、自動的に **Viewing** 状態（薄いグレー）になり、実行結果を安全に閲覧できること。

##### Case 43-6: リセット (Live/Viewing -> Idle)
*   **操作**: `Reset` ボタンをクリック。
*   **確認事項**:
    1.  背景色が **薄いグレー** (Viewing) になり、エディタがクリアされること。
    2.  ナビゲーションボタンが非活性化されること。
