...
try {
    ...
    var diagram = diagramEditor.createStatemachineDiagram(project, "TestModel_No19_InternalChoice");
    ...
    state0Pres.setWidth(300);
    state0Pres.setHeight(200);
    ...
    // Choice (inside State0)
    var choicePres = diagramEditor.createChoicePseudostate(state0Pres, new Point2D.Double(250, 150));

    // StateB (outside State0)
    var stateBPres = diagramEditor.createState("StateB", null, new Point2D.Double(450, 50));
    stateBPres.getModel().setEntry("entB");

    // StateC (outside State0)
    var stateCPres = diagramEditor.createState("StateC", null, new Point2D.Double(450, 150));
    ...
    print("Test Model No.19 (Internal Choice) created successfully.");
